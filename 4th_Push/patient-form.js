...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  async function loadStates() {
    try {
      const response = await fetch("/4th_Push/states.json");
      if (!response.ok) {
      ...
      alert("Unable to load state list. Please refresh the page.");
    }
  }
  loadStates();
});

// COOKIE to remember patient name
...
// COOKIE RESTORE — Display welcome message if cookie exists
document.addEventListener("DOMContentLoaded", function () {
  const savedName = getCookie("patientName");
  const welcome = document.getElementById("welcomeBackMsg");
  ...
  if (savedName !== "") {
    ...
    if (first) first.value = savedName;

    // Show "Not you? Start new patient" link
    if (newUserLink) {
      newUserLink.style.display = "inline";
    }
  }
});
...
(function () {
  ...
  if (savedData) {
    try {
      ...
      Object.keys(data).forEach(key => {
        if (["hx", "gender", "vaccinated", "insurance"].includes(key)) return;

        const el = document.getElementById(key);
        if (el) el.value = data[key];
      });

      // Restore HX checkboxes
      if (data.hx) {
        document.querySelectorAll("input[name='hx']").forEach(box => {
          if (data.hx[box.value] !== undefined) {
            box.checked = data.hx[box.value];
          }
        });
      }
      ...
      // Restore vaccinated radio
      if (data.vaccinated) {
        const v = document.querySelector(`input[name='vaccinated'][value='${data.vaccinated}']`);
        if (v) v.checked = true;
      }

      // Restore insurance radio
      if (data.insurance) {
        const ins = document.querySelector(`input[name='insurance'][value='${data.insurance}']`);
  ...
  form.addEventListener('submit', function (evt) {
    ...
    // REMEMBER ME — Save first name in cookie for 48 hours
const remember = document.getElementById("rememberMe");
const firstName = document.getElementById("firstName").value.trim();

if (remember && remember.checked && firstName !== "") {
    setCookie("patientName", firstName, 48);
} else {
    deleteCookie("patientName");
}

// LOCALSTORAGE — Save all form data except SSN & Passwords


let saveObj = {}; // Build an object with field values
const fieldsToSave = [
  "firstName", "middleinitile", "lastName",
  "dob", "email", "phone",
  ...
  if (el) saveObj[id] = el.value.trim();
});


    // Gender (radio)
    const gender = document.querySelector("input[name='gender']:checked");
    saveObj.gender = gender ? gender.value : "";

    // Vaccinated (radio)
    const vac = document.querySelector("input[name='vaccinated']:checked");
    saveObj.vaccinated = vac ? vac.value : "";

    // Insurance Type (radio)
    const ins = document.querySelector("input[name='insurance']:checked");
    saveObj.insurance = ins ? ins.value : "";


// Get checkboxes (hx)
const hxBoxes = document.querySelectorAll("input[name='hx']");
saveObj.hx = {};
hxBoxes.forEach(box => {
  saveObj.hx[box.value] = box.checked;
});

// Save to localStorage
localStorage.setItem("patientFormData", JSON.stringify(saveObj));
...
// ---------------- NEW PATIENT RESET (Assignment 4) ----------------
